...
$records_at_page = 15;
$count_sql = $connect->query ("SELECT * FROM users_messages WHERE to_index='$parent_index'");
...
$stmt_messages_read = $connect->query("UPDATE users_messages SET msg_read='1' WHERE to_index='$parent_index'");

$stmt_messages = $connect->query("SELECT * FROM users_messages WHERE to_index='$parent_index' ORDER BY id DESC LIMIT $start, $end");
...
if ($next <= $pages_count) {
  echo '<a href="messages.php?page=' . $next . '" class="btn btn-default p_right">'.$lang ['next'].' <i class="glyphicon glyphicon-arrow-right"></i></a>' ;
}

if ($prev > 0) {
  echo '<a href="messages.php?page=' . $prev . '" class="btn btn-default p_left"><i class="glyphicon glyphicon-arrow-left"></i> '.$lang ['prev'].'</a>' ;
}
 ?>
