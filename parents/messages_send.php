...
$stmt_messages = $connect->prepare("SELECT * FROM users_messages WHERE author_index='$parent_index' AND id=:id ");
...
$records_at_page = 20;
$count_sql = $connect->query ("SELECT * FROM users_messages WHERE author_index='$parent_index' AND hide_msg='0'");
...
if (($page > $pages_count) || ($page <= 0 )) {
  echo'<div class="container mainbg"><br><a class="return" href="index.php" style="margin-left:20px;"><i class="glyphicon glyphicon-arrow-left"></i> '. $lang ['return'].'</a> <br><br><div class="alert alert-info center"><p>'.$lang ["no_outgoing_messages"].'</p></div><br></div>';
}
...
if ($_GET['token'] == $_SESSION['token']) {
  ...
  $stmt_delete = $connect->prepare("UPDATE users_messages SET hide_msg='1' WHERE id=:id AND author_index='$parent_index'");
...
$stmt_messages = $connect->query("SELECT * FROM users_messages WHERE author_index='$parent_index' AND hide_msg='0' ORDER BY id DESC LIMIT $start, $end");
