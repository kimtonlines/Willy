...
$stmt_get_student = $connect->query("SELECT * FROM students_users WHERE parent_index='$parent_index' ");
    ...
    echo '<h1 class="h1_title">transport</h1>
...
while ($stmt_students_row = $stmt_get_student->fetch()) {
      ...
      $fetch_students_index = $stmt_students_row ['student_index'];
        echo '<tr>
          ...
          <td><a href="transport.php?student='.$fetch_students_index.'"><i class="fa fa-bus large" style="font-size: 28px; color: #c55;"></i></a></td>
        </tr>';
  }
...
if (isset($_GET['student'])) {
 ...
 ?>
    ...
    <h1 class="h1_title"><?php echo $lang ['transport']; ?> (<?php echo $fetch_students_name; ?>)</h1>
...
<?php 
  ...
  $stmt_transport = $connect->prepare("SELECT * FROM transport WHERE class_name='$fetch_students_class'");
