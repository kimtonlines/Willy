...
$stmt_messages = $connect->prepare("SELECT * FROM users_messages WHERE author_index='$my_student_index' AND id=:id ");
...
$records_at_page = 20;
$count_sql = $connect->query ("SELECT * FROM users_messages WHERE author_index='$my_student_index' AND hide_msg='0'");
...
if ($_GET['token'] == $_SESSION['token']) {
  ...
  $stmt_delete = $connect->prepare("UPDATE users_messages SET hide_msg='1' WHERE id=:id AND author_index='$my_student_index'");
...
$stmt_messages = $connect->query("SELECT * FROM users_messages WHERE author_index='$my_student_index' AND hide_msg='0' ORDER BY id DESC LIMIT $start, $end");
