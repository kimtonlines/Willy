...
$my_number = $row_allInfo ['registration_num'];
$my_class = $row_allInfo ['student_class'];
$my_gender = $row_allInfo ['sex'];
$my_birthday = $row_allInfo ['birthday'];
$my_address = $row_allInfo ['address'];
$my_phone = $row_allInfo ['phone'];
$my_email = $row_allInfo ['email'];

$my_guardian = $row_allInfo ['parent_index'];

$stmt_get_parent = $connect->query("SELECT * FROM parents_users WHERE parent_index='$my_guardian' ");
...
<a class="return" href="index.php"><i class="glyphicon glyphicon-arrow-left"></i> <?php echo $lang ['return']; ?></a><br><br>
  ...
  <div class="panel panel-info">
    <div class="panel-heading center">
      <img src="../uploads/students/<?php if (empty($my_image)) {echo 'no_image.png';} else {echo $my_image;} ?>" class="img-thumbnail" width="150px">
      
      <h2><?php echo $my_fullname; ?> <br> <span><?php echo $lang ['student']; ?></span></h2>

    </div>
    <div class="panel-body">
<?php 
if (isset($_GET['update']) == "success") {
  echo "<br><div class='alert alert-success center' style='width: 90%; margin: auto;'><p>".$lang ['Modified_successfully']."</p></div><br><br>"; 
 ...
 <div class="clear"></div>
    ...
    <div class="col-md-6">
      <label><?php echo $lang ['registration_number']; ?>  :</label> <span><?php echo $my_number; ?></span><br><br>
      <label><?php echo $lang ['class']; ?>  :</label> <span><?php echo $my_class; ?></span><br><br>
      <label><?php echo $lang ['gender']; ?>  :</label> <span><?php echo $my_gender; ?></span><br><br>
      <label><?php echo $lang ['birth_date']; ?>  :</label> <span><?php echo $my_birthday; ?></span><br><br>
    </div>
    <div class="col-md-6">
...
if (isset($_POST['update_info'])) {
  ...
  $update_email = htmlspecialchars($_POST['email']);
   ...
   $stmt_update_info = $connect->prepare("UPDATE students_users SET email=:email, phone=:phone, address=:address WHERE student_index='$my_student_index'");
...
if (isset($_POST['update_passe'])) {
  ...
  $stmt_get = $connect->query("SELECT * FROM students_users WHERE student_index='$my_student_index' ");
  ...
  if ($old_pass == $fetch_pass) {
      ...
      $stmt_update_pass = $connect->prepare("UPDATE students_users SET password=:password WHERE student_index='$my_student_index'");
