...
if (isset($_SESSION ['teacher_index'])) {
  $teacher_index = $_SESSION ['teacher_index'];
}
...
$stmt_get_allInfo = $connect->query("SELECT * FROM teachers_users WHERE teacher_index='$teacher_index' ");
...
if (isset($_POST['submit'])) {
  ...
  $message_date = date("d/m/Y");
   ...
   $stmt_messages = $connect->prepare("INSERT INTO users_messages (author_index, author_name, to_index, type, subject, message, date) VALUES ('$teacher_index', :author_name, :to_index, '$message_from', :subject, :message, '$message_date')");
 ...
 ?>
  ...
  <form id="formID" method="post" action="">
              ...
              <div class="input-group">
                  ...
                  <option selected="selected" value="teacher"><?php echo $lang ['teacher']; ?></option>
                  </select>
              </div><br>
        ...
        <label class=""><?php echo $lang ['message']; ?> <span style="color:red; font-weight: bold; font-family: Arial, sans-serif ;">(*)</span></label>
              <div class="input-group">
                ...
                <textarea id="area1" class="form-control validate[required]" name="message"><?php echo htmlspecialchars(!empty($_POST['message'])) ? htmlspecialchars($_POST['message']) : ''?></textarea>
              </div><br>
